...
impl BTreeProps {
    ...
    fn split_child<T: Ord + Copy + Default>(&self, parent: &mut Node<T>, child_index: usize) {
        ...
        let right_key = match child.keys.split_off(self.mid_key_index).split_first() {
        ...
        let new_child_node: Node<T> = Node::new(self.degree, Some(right_key), right_children);
